...
# Ülkelere Göre Gelir Dağılımı Grafiği
st.write("### Ülkelere Göre Gelir Dağılımı")
if 'Country' in df.columns:
    gelir_by_ulke = df.groupby('Country')['UnitPrice'].sum()
    fig_ulke_gelir = px.bar(gelir_by_ulke,
                            x=gelir_by_ulke.index,
                            y=gelir_by_ulke.values,
                            labels={'x': 'Ülke', 'y': 'Gelir'},
                            title='Ülkelere Göre Gelir Dağılımı',
                            color=gelir_by_ulke.index)
    fig_ulke_gelir.update_layout(
        plot_bgcolor="white",
        paper_bgcolor="white",
        font=dict(size=14, color="black"),
        title_font=dict(size=16, family="Arial, sans-serif"),
    ...
    st.plotly_chart(fig_ulke_gelir, use_container_width=True)
else:
